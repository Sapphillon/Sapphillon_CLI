var __create = Object.create;
  ...
  var import_npm_lodash_4_17 = __toESM(require_lodash());
  function toCamelCase(text) {
    return import_npm_lodash_4_17.default.camelCase(text);
  }
  function toSnakeCase(text) {
    return import_npm_lodash_4_17.default.snakeCase(text);
  }
  function toKebabCase(text) {
    return import_npm_lodash_4_17.default.kebabCase(text);
  }
  var import_npm_lodash_4_172 = __toESM(require_lodash());
  function truncateText(text, length) {
    return import_npm_lodash_4_172.default.truncate(text, { length, omission: "..." });
  }
  function toTitleCase(text) {
    return import_npm_lodash_4_172.default.startCase(text);
  }
  function normalizeWhitespace(text) {
    return import_npm_lodash_4_172.default.words(text).join(" ");
  }
  var import_npm_lodash_4_173 = __toESM(require_lodash());
  function countWords(text) {
    return import_npm_lodash_4_173.default.words(text).length;
  }
  function getWordFrequency(text) {
    const words = import_npm_lodash_4_173.default.words(text.toLowerCase());
    return import_npm_lodash_4_173.default.countBy(words);
  }
  function toCamelCase2(text) {
    return toCamelCase(text);
  }
  function toSnakeCase2(text) {
  ...
  function getWordFrequency2(text) {
    return getWordFrequency(text);
  }

Sapphillon.Package = {
  meta: {
    name: "Text Utils Plugin",
    version: "1.0.0",
    description: "A plugin for text manipulation using lodash library.",
    author_id: "app.sapphillon",
    package_id: "app.sapphillon.text-utils-plugin"
  },
  functions: {
    toCamelCase: {
      handler: toCamelCase,
      ...
      returns: [
        { type: "string", idx: 0, description: "キャメルケースに変換された文字列" }
      ]
    },
    toSnakeCase: {
      handler: toSnakeCase,
      permissions: [],
      description: "テキストをスネークケースに変換します。",
      parameters: [
        { name: "text", idx: 0, type: "string", description: "変換する文字列" }
      ],
      returns: [
        { type: "string", idx: 0, description: "スネークケースに変換された文字列" }
      ]
    },
    toKebabCase: {
      handler: toKebabCase,
      permissions: [],
      description: "テキストをケバブケースに変換します。",
      parameters: [
        { name: "text", idx: 0, type: "string", description: "変換する文字列" }
      ],
      returns: [
        { type: "string", idx: 0, description: "ケバブケースに変換された文字列" }
      ]
    },
    truncateText: {
      handler: truncateText,
      permissions: [],
      description: "テキストを指定した長さに切り詰めます。",
      parameters: [
        { name: "text", idx: 0, type: "string", description: "切り詰める文字列" },
        { name: "length", idx: 1, type: "number", description: "最大文字数" }
      ],
      returns: [
        { type: "string", idx: 0, description: "切り詰められた文字列" }
      ]
    },
    toTitleCase: {
      handler: toTitleCase,
      permissions: [],
      description: "テキストの各単語の先頭を大文字にします。",
      parameters: [
        { name: "text", idx: 0, type: "string", description: "変換する文字列" }
      ],
      returns: [
        { type: "string", idx: 0, description: "タイトルケースに変換された文字列" }
      ]
    },
    normalizeWhitespace: {
      handler: normalizeWhitespace,
      permissions: [],
      description: "テキストから余分な空白を除去します。",
      parameters: [
        { name: "text", idx: 0, type: "string", description: "処理する文字列" }
      ],
      returns: [
        { type: "string", idx: 0, description: "空白が正規化された文字列" }
      ]
    },
    countWords: {
      handler: countWords,
      permissions: [],
      description: "テキスト内の単語数をカウントします。",
      parameters: [
        { name: "text", idx: 0, type: "string", description: "分析する文字列" }
      ],
      returns: [
        { type: "number", idx: 0, description: "単語数" }
      ]
    },
    getWordFrequency: {
      handler: getWordFrequency,
      permissions: [],
      description: "テキスト内の各単語の出現回数を集計します。",
      parameters: [
        { name: "text", idx: 0, type: "string", description: "分析する文字列" }
      ],
      returns: [
        { type: "Object", idx: 0, description: "単語と出現回数のオブジェクト" }
      ]
    }
  }
};
